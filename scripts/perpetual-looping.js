...
function createPerpetualLoopingLayout(flowers) {
  ...
  console.log(`Found ${categoryFlowers.length} flowers for Perpetual Looping`);
  ...
  // Golden spiral positioning (compact spiral for looping) with full container spread
  const GOLDEN = 137.50776405 * (Math.PI / 180);
  const aBase = Math.min(containerRect.width, containerRect.height) * 0.35; // Compact for looping effect
