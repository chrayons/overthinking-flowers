...
function createTemporalDisconnectionLayout(flowers) {
  ...
  console.log(`Found ${categoryFlowers.length} flowers for Temporal Disconnection`);
  ...
  // Golden spiral positioning (elongated spiral for temporal disconnection) with full container spread
  const GOLDEN = 137.50776405 * (Math.PI / 180);
  const aBase = Math.min(containerRect.width, containerRect.height) * 0.45; // Elongated spread
  const bBase = 0.4 * aBase; // More elongated
  const placedFlowers = [];
