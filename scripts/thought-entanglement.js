...
function createThoughtEntanglementLayout(flowers) {
  ...
  console.log(`Found ${categoryFlowers.length} flowers for Thought Entanglement`);
  ...
  // Golden spiral positioning (wider spiral for thought entanglement) with full container spread
  const GOLDEN = 137.50776405 * (Math.PI / 180);
  const aBase = Math.min(containerRect.width, containerRect.height) * 0.45; // Wider for entanglement effect
