...
function createPerceptualBarriersLayout(flowers) {
  ...
  console.log(`Found ${categoryFlowers.length} flowers for Perceptual Barriers`);
  ...
  // Golden spiral positioning (standard spiral) with full container spread
  const GOLDEN = 137.50776405 * (Math.PI / 180);
  const aBase = Math.min(containerRect.width, containerRect.height) * 0.4; // Standard spread
